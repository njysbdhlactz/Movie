...
<%
  ...
  MovieDao movieDao = new MovieDao();
  MovieTO movieTO = movieDao.getMovieDetailById(Integer.valueOf(movieId));
...
<body>
	<div class="information" id="<%=movieId%>">
		<div class="title">
			<span class="movieTitle"><%=movieTO.getTitle()%> <span
				class="movieRelease"><%='('+movieTO.getDate().substring(0,4)+')'%>
			</span> </span><br>
		</div>
		<div class="clear"></div>
		<div style="margin-top: 25px;">
			<div class="leftInfoPanel">
				<div style="font-weight:bolder;">
					Rent for $<%=movieTO.getPrice()%></div>

				<%
					if (movieTO.getPosterPath().equalsIgnoreCase("") || !useMoviePosters) {
				%>
				<div class="videoContainer" style="position:relative;">
					<img width="184px" height="276px" src="images/genericfilm.png"
						name="genericfilm.png" style="display: block; " class="reflected"></img>
					<div class="tit3"
						style="top:150px;left:-20px;padding:5px;display:table;width:140px;height:90px;margin-top:0px;">
					...
					<div class="videoContainer">
						...
						<br> <a href="#movie" class="playButton" movieid="<%= movieId %>"></a>
					</div>
					<div>
						<%
							double avg;
						...
						%>
						<input name="star1" type="radio" class="starMovie star required"
							value="1"
							<%if (avg > 0 && avg <= 1.5 ) out.print("checked='checked'");%> />
						<input name="star1" type="radio" class="starMovie star" value="2"
							<%if (avg > 1.5 && avg <= 2.5 ) out.print("checked='checked'");%> />
						<input name="star1" type="radio" class="starMovie star" value="3"
							<%if (avg > 2.5 && avg <= 3.5 ) out.print("checked='checked'");%> />
						<input name="star1" type="radio" class="starMovie star" value="4"
							<%if (avg > 3.5 && avg <= 4.5 ) out.print("checked='checked'");%> />
						<input name="star1" type="radio" class="starMovie star" value="5"
							<%if (avg > 4.5) out.print("checked='checked'");%> />
					</div>
				</div>
				...
				<div class="rightInfoPanel">
					<div>
						<label>Overview: </label> <span><%=overview%></span>
					</div>
					<div>
						<label>Cast: </label> <span><%=shortActors%></span>
					</div>
					<div>
						<label>Director: </label> <span><%=director%></span>
					</div>
					<div>
						<label>Writer: </label> <span><%=writer%></span>
					</div>
				</div>
			</div>
		</div>
